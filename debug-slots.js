...
// Database config (same as API)
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgresql://tabletech_owner:LqRJelyNdLCW@ep-frosty-heart-a27xwzd1.eu-central-1.aws.neon.tech/tabletech?sslmode=require',
  ssl: {
...
async function debugSlots() {
  try {
    ...
    const slotsQuery = `
      ...
      ORDER BY start_time
    `;
    const slotsResult = await pool.query(slotsQuery, [dayOfWeek]);
    ...
    `;
    const appointmentsResult = await pool.query(appointmentsQuery, [testDate]);
    ...
    const availableSlotsQuery = `SELECT * FROM get_available_slots($1::date)`;
    const availableResult = await pool.query(availableSlotsQuery, [testDate]);
    ...
    const blockedQuery = `SELECT * FROM blocked_dates WHERE blocked_date = $1`;
    const blockedResult = await pool.query(blockedQuery, [testDate]);
    ...
    console.log('\n🔄 Test na cleanup:');
    const finalTest = await pool.query(availableSlotsQuery, [testDate]);
