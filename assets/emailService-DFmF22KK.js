const p=async e=>{try{const r=`TT-${Date.now()}-${Math.random().toString(36).substr(2,9)}`,l=`${e.date}-${e.time}`,a=JSON.parse(localStorage.getItem("tabletech-booked-slots")||"{}");if(a[l])throw new Error("Deze tijd is al gereserveerd. Kies een andere tijd.");a[l]={bookingId:r,customerName:e.name,customerEmail:e.email,bookedAt:new Date().toISOString()},localStorage.setItem("tabletech-booked-slots",JSON.stringify(a));const d={to:"info@tabletech.nl",subject:`🔥 NIEUWE AFSPRAAK - ${e.name} (${e.date} ${e.time})`,bookingId:r,appointmentData:e};try{const o=JSON.parse(localStorage.getItem("tabletech-appointments")||"[]");o.push({...d,timestamp:new Date().toISOString(),id:Date.now()}),localStorage.setItem("tabletech-appointments",JSON.stringify(o))}catch{}let n=!1;try{const o=`
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
<body style="margin: 0; padding: 0; background-color: #f6f9fc; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Ubuntu, sans-serif;">
  <div style="background-color: #f6f9fc; padding: 20px 0; margin: 0 auto; width: 100%;">
    ...
    <div style="background-color: #f7fafc; padding: 40px 20px; text-align: center; border-top: 1px solid #e2e8f0; margin-top: 40px;">
      ...
      `,i=new FormData;i.append("_to",e.email),i.append("_subject",`✅ Bevestiging: Je TableTech afspraak op ${e.date}`),i.append("_template","table"),i.append("_captcha","false"),i.append("_replyto","info@tabletech.nl"),i.append("_format","html"),i.append("_cc","info@tabletech.nl"),i.append("klant_naam",e.name),i.append("afspraak_datum",e.date),i.append("afspraak_tijd",e.time),i.append("message",c),(await fetch("https://formsubmit.co/ajax/"+e.email,{method:"POST",body:i,headers:{Accept:"application/json"}})).ok}catch{}if(!n)try{const o=await fetch("https://formspree.io/f/xpwzgzrd",{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({email:e.email,naam:e.name,telefoon:e.phone,restaurant:e.restaurant||"Niet opgegeven",datum:e.date,tijd:e.time,bericht:e.message||"Geen specifiek bericht",_subject:`🔥 NIEUWE AFSPRAAK - ${e.name} (${e.date})`,_replyto:e.email})});if(o.ok)n=!0;else throw new Error(`Formspree failed: ${o.status}`)}catch{}if(!n)try{const o=new FormData;o.append("form-name","tabletech-appointments"),o.append("naam",e.name),o.append("email",e.email),o.append("telefoon",e.phone),o.append("restaurant",e.restaurant||"Niet opgegeven"),o.append("datum",e.date),o.append("tijd",e.time),o.append("bericht",e.message||"Geen specifiek bericht");const t=await fetch("/",{method:"POST",body:o});if(t.ok)n=!0;else throw new Error(`Netlify Forms failed: ${t.status}`)}catch{}if(!n){const o={timestamp:new Date().toISOString(),status:"MANUAL_FOLLOW_UP_REQUIRED",appointmentData:e,priority:"HIGH",action_required:"Contact customer directly within 24 hours"};try{const t=JSON.parse(localStorage.getItem("tabletech-failed-emails")||"[]");t.push(o),localStorage.setItem("tabletech-failed-emails",JSON.stringify(t))}catch{}}return n}catch(r){if(r instanceof Error&&r.message.includes("al gereserveerd"))throw r;return!1}};export{p as sendAppointmentEmail};
